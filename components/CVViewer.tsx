...
const CVViewer: React.FC<CVViewerProps> = ({ language, onNavigate }) => {
  ...
  const heroImage = "https://app.leonardo.ai/img/new-background.webp";

  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      {/* Header */}
      <section className="space-y-3 pt-4">
        <h1 className="text-5xl md:text-6xl font-black mb-4 leading-tight text-slate-900 dark:text-white">
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          <a href={`mailto:${data.email}`} className="glass px-3 py-1.5 rounded-full text-xs text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-white transition-colors">✉️ {data.email}</a>
          <span className="glass px-3 py-1.5 rounded-full text-xs text-slate-600 dark:text-slate-300">📞 {data.phone}</span>
          <span className="glass px-3 py-1.5 rounded-full text-xs text-slate-600 dark:text-slate-300">📍 {data.location}</span>
          <button 
            onClick={() => onNavigate(AppView.PORTFOLIO)} 
            className="glass px-3 py-1.5 rounded-full text-xs text-indigo-600 dark:text-indigo-300 hover:text-indigo-800 dark:hover:text-indigo-100 transition-colors font-semibold"
          >
            🌐 {labels.portfolio}
          </button>
        </div>
      ...
      <section className="glass p-6 rounded-[1.5rem]">
        <h2 className="text-lg font-bold mb-3 text-slate-900 dark:text-white flex items-center">
          <span className="mr-2">👤</span> {labels.summary}
        </h2>
      ...
      <div className="grid md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-8 space-y-8">
            ...
            <div className="space-y-8">
              {data.experience.map((job, idx) => (
                <div key={idx} className="relative pl-6 border-l border-slate-200 dark:border-[#282c42]">
                  <div className="absolute -left-[5px] top-0 w-2.5 h-2.5 rounded-full bg-white shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 gap-1">
                    <div className="flex-column flex-wrap gap-4">
                      <h3 className="text-lg mb-2 font-bold text-slate-900 dark:text-white leading-tight">{job.role}</h3>
                      <p className="text-indigo-600 dark:text-indigo-400 font-semibold text-sm">{job.company}</p>
                    </div>
                    <span className="text-[12px] text-slate-500 dark:text-slate-400 font-mono glass px-2 py-1 rounded-lg self-start sm:self-center">
                      {job.period}
                    </span>
                  </div>
                  <ul className="space-y-2 mt-3">
                    {job.highlights.map((point, i) => (
                      <li key={i} className="text-slate-600 dark:text-slate-400 text-s flex items-start leading-relaxed">
          ...
          <section>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-[#282c42] pb-2 mb-6 flex items-center">
              <span className="mr-2">🗣️</span> {labels.references}
            </h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {data.references.map((ref, i) => (
                <div key={i} className="glass p-4 rounded-2xl">
                  ...
                  <p className="text-[12px] text-slate-500 font-mono break-all">{ref.contact}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="md:col-span-4 space-y-8">
          <section>
            <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center">
              <span className="mr-2">🛠️</span> {labels.skills}
            </h2>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {data.skills.tech.map((s, i) => (
                    <span key={i} className="px-2 py-1 bg-indigo-500/10 border border-slate-200 dark:border-[#282c42] text-indigo-700 dark:text-indigo-300 rounded-lg text-[12px] font-semibold">{s}</span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {data.skills.design.map((s, i) => (
                    <span key={i} className="px-2 py-1 bg-purple-500/10 border border-slate-200 dark:border-[#282c42] text-purple-700 dark:text-purple-300 rounded-lg text-[12px] font-semibold">{s}</span>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {data.skills.platforms.map((s, i) => (
                    <span key={i} className="px-2 py-1 bg-emerald-500/10 border border-slate-200 dark:border-[#282c42] text-emerald-700 dark:text-emerald-300 rounded-lg text-[12px] font-semibold">{s}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <section>
            ...
            <div className="space-y-3">
              {data.education.map((edu, i) => (
                <div key={i} className="glass p-4 rounded-2xl transition-transform hover:scale-[1.01]">
                  <p className="font-bold text-slate-900 dark:text-white text-xs leading-tight mb-1">{edu.degree}</p>
                  <p className="text-[12px] text-indigo-600 dark:text-indigo-400 font-medium">{edu.school}</p>
                  <p className="text-[12px] text-slate-500 dark:text-slate-500 mt-1 font-mono uppercase tracking-widest">{edu.year}</p>
                </div>
              ))}
            </div>
          </section>
