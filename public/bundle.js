(function () {
	...
	const appearFunction = () => {
		...
		trabajos.forEach((element, index) => {
			setTimeout(() => {
				element.classList.add('trabajos__trabajo--visible');					
			}, 300 * index);
		});
	};
	...
	const observer = new IntersectionObserver(
		(entries) => {
			...
			//se colocan los parametros en este segundo objeto
			rootMargin: '0px 0px 0px 0px',
			//threshold indica el porcentaje de aparicion
			threshold: 0.3,
		}	
	);
  ...
  .replace(/ /g, '.')}`;
	...
	ventaTrabajo.addEventListener('click', (e) => {
		if(e.target.closest('[data-action="cerrar-ventana"]')) {
			ventaTrabajo.querySelector('.ventana__contenedor-imagen').scrollIntoView({
				inline: "start",
				block: "nearest"
			});
			ventaTrabajo.classList.remove('ventana--active');
		}
	...
	const ultimoCursor = document.querySelector('.hero__titulo:last-child #cursor');
	const animarTexto = (element) => {
		const numeroDeLetras = element.dataset.texto;
		const cursor = element.querySelector('#cursor');

		
		cursor.classList.add('hero__cursor--visible');
		
		for (let i = 0; i < numeroDeLetras.length; i++) {
			setTimeout(() => {
				const letras = numeroDeLetras[i];
		...
		const cursores = element.querySelector('#cursor');
		setTimeout(() => {
			cursores.style.display = 'none';
			ultimoCursor.style.display = 'block';
		}, numeroDeLetras.length * 100);
		
		
		return new Promise((resolve) => {
			ultimoCursor.classList.add('hero__cursor--active');
			setTimeout(resolve, numeroDeLetras.length * 100);
		});
	};
	...
	*/
	...
	window.addEventListener('load', async() => {
		...
		await animarTexto(document.querySelector('.hero__titulo--tres'));

		const burbujas = document.querySelectorAll('.hero__burbuja');
		for (let i = 0; i < burbujas.length; i++) {
			burbujas[i].classList.add('hero__burbuja--active-' + parseFloat(i + 1));
		}
